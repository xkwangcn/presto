...
public class TestPrometheusClient
{
            ...
            throws Exception
    {
        ...
        PrometheusClient client = new PrometheusClient(new PrometheusConfig().setMetadata(metadata), CATALOG_CODEC);
        assertEquals(client.getSchemaNames(), ImmutableSet.of("prometheus", "tpch"));
        assertEquals(client.getTableNames("prometheus"), ImmutableSet.of("numbers"));
        assertEquals(client.getTableNames("tpch"), ImmutableSet.of("orders", "lineitem"));

        PrometheusTable table = client.getTable("prometheus", "numbers");
        assertNotNull(table, "table is null");
        assertEquals(table.getName(), "numbers");
        assertEquals(table.getColumns(), ImmutableList.of(new PrometheusColumn("text", createUnboundedVarcharType()), new PrometheusColumn("value", BIGINT)));
